...
#### システム構成
- **フロントエンド**: dify-web（React.js）- Dify公式が提供しているものを使用
- **バックエンド**: dify-api（Python Flask）- Dify公式が提供しているものを使用
- **ワーカー**: dify-worker（バックグラウンド処理）- Dify公式が提供しているものを使用
- **キャッシュ**: Redis（セッション・キャッシュ管理）- Dify公式が提供しているものを使用
- **データベース**: PostgreSQL - Dify公式が提供しているものを使用
- **リバースプロキシ**: nginx（SSL終端・ALB連携対応）- カスタマイズ
- **プロキシ**: ssrf_proxy（アウトバウンド通信制御）- カスタマイズ
...
**ドメイン・DNS構成**
- Route53にて`s‍st-cloudmng.com`を親ドメインとし、ワークショップ用サブドメインとして`20250918-dify.sst-cloudmng.com`を利用する。
- DNS管理はRoute53で実施し、ALBのDNS名にCNAMEレコードで紐付ける。
...
| リソース名 | 数量 | 用途 |
|---|---|---|
| Route 53 | 1 | DNS管理（`sst-cloudmng.com`配下に`20250918-dify.sst-cloudmng.com`サブドメインを設定） |
| AWS Certificate Manager | 1 | SSL証明書管理（`20250918-dify.sst-cloudmng.com`用） |
...
| セキュリティグループ分離 | 役割ベース | ALB・EC2・管理用 |
...
ワークショップ環境の運用監視は、基本的なヘルスチェックとログ確認による監視体制とする。

#### 監視項目
| 監視項目 | 監視方法 | 確認頻度 | 対応方法 |
|---|---|---|---|
| Difyアプリケーション | ALBヘルスチェック | 継続的 | 自動復旧・手動対応 |
| EC2インスタンス | 手動確認 | ワークショップ開始前 | 手動対応 |
| Dockerコンテナ | 手動確認 | ワークショップ開始前 | 手動対応 |

#### ログ管理
| ログ種別 | 保存先 | 保持期間 | 用途 |
|----------|--------|----------|------|
| ALBアクセスログ | S3バケット | ワークショップ期間中 | アクセス解析 |
| Difyアプリケーションログ | EC2ローカル | ワークショップ期間中 | アプリケーション監視 |
| システムログ | EC2ローカル | ワークショップ期間中 | システム監視 |