...
学習内容はDifyの操作方法とAIアプリケーション作成に特化し、クラウドインフラの知識は不要とする。
...
インフラ部分は運用者が管理し、学習者はDifyの機能習得に集中できる環境を提供する。
...
#### ①教育効果
- Difyプラットフォームを使用したAIアプリケーション開発の実習
- ノーコード/ローコードでのAI活用手法の習得
- 実際のAI機能（Bedrock Claude 3.7 Sonnet）を使った実践的な学習

#### ②運用効率
- 学生がDify学習に集中できるよう、インフラ管理は運用者が担当
- 安定したワークショップ環境の提供
- 学習に必要な機能のみを提供し、複雑性を排除
...
- **プライベートサブネット**: EC2配置用
- **インターネット接続**: ALB（パブリック）、NAT Gateway（プライベート）
...
| Application Load Balancer | 1 | 外部からのアクセス制御・負荷分散 |
...
| Amazon Bedrock | － | リージョン依存 | 生成AI機能<br>Claude 3.7 Sonnetによる学習用AI機能 |
